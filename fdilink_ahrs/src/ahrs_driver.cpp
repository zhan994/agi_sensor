  ...
  while (ros::ok())
  {
    ...
    else if (head_type[0] == TYPE_INSGPS)
    {
      ...
      odom_msg.twist.twist.linear.x =
          insgps_frame_.frame.data.data_pack.Velocity_North; // Speed in the X direction //X方向速度
      odom_msg.twist.twist.linear.y =
          insgps_frame_.frame.data.data_pack.Velocity_East; // Speed in the Y direction //Y方向速度
